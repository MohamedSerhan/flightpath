...
`is-a.dev` runs a free subdomain registry for developers. With one PR
to their repo and one repo variable here, the site moves from
`mohamedserhan.github.io/flightpath/` to `flightpath.is-a.dev`. Full
walkthrough in [research/05-custom-domain.md](research/05-custom-domain.md).

The repo-side wiring is already in place — `CUSTOM_DOMAIN` repo variable
controls `VITE_BASE` and CNAME-file generation in the workflow.
...
- [Custom domain via is-a.dev](research/05-custom-domain.md)
